...
if($type -ne "win10" -and $type -ne "ubuntu" -and $type -ne "win7" -and $type -ne "server2016")
{
...
if($null -eq $resourceGroup)
{
    ...
    $resourceGroup = New-AzResourceGroup -Name $resourceGroupName -Location "westeurope"
}
...
if($null -eq $vnet)
{
    ...
    $subnet = New-AzVirtualNetworkSubnetConfig -Name "default" -AddressPrefix $subnetPrefix
    $vnet = New-AzVirtualNetwork -Name $networkName -ResourceGroupName $resourceGroupName -Location $location -AddressPrefix $addressPrefix -Subnet $subnet
}
...
if($nsg -eq $null)
{
    $sshRule = New-AzNetworkSecurityRuleConfig -Name ssh-rule -Description "Allow SSH" -Access Allow -Protocol Tcp -Direction Inbound -Priority 101 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 22
    $rdpRule = New-AzNetworkSecurityRuleConfig -Name rdp-rule -Description "Allow RDP" -Access Allow -Protocol Tcp -Direction Inbound -Priority 102 -SourceAddressPrefix Internet -SourcePortRange * -DestinationAddressPrefix * -DestinationPortRange 3389
    $nsg = New-AzNetworkSecurityGroup -Name $networkSecurityGroupName -ResourceGroupName $resourceGroupName -Location $location -SecurityRules $sshRule,$rdpRule
}

$publicIPAddressName = $vmName+"-pubip-"+$randomText
$publicIP = New-AzPublicIpAddress -Name $publicIPAddressName -ResourceGroupName $resourceGroupName -Location $location -AllocationMethod Dynamic -DomainNameLabel $vmName
...
New-AzResource -Location $location -ResourceId $scheduledShutdownResourceId -Properties $autoShutdownProp -Force
